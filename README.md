# Container ready for wordpress development.
### Please check my shellscript wpdev to automatically provide a wordpress development environment. This script uses this container.

## Added wp-config.php to:
## CentOS-7 with supervisord launcher, SSH server, shellinabox and httpd server php 5.6.x | Docker

This is a CentOS-7 Docker with supervisor, SSH server, shellinabox, httpd server, php 5.6.x and wp-config.php
...
### (prerequisite is to have properly run a mysql container. See orboan/dcsss-mariadb)

#### docker run -p 8000:80 -p 2222:22 -p 9001:9001 -e USER=myusername -e PASSWORD=mypassword -e SHELLINABOX_PORT=9103 -p 9103:4200 -e MYSQL_HOST=mysql -e MYSQL_DATABASE=wpdb -e MYSQL_USER=wpuser -e MYSQL_PASSWORD=wordpress -e WP_URL="http://localhost" -d -v ~/data:/data orboan/dcsss-httpd-wpdev
...
In the /data volume the DocumentRoot (www/html) will be created. From that point you should download the latest wordpress and unpack it into this directory. This means even if the containers are destroyed, the plugin/theme files you created will be preserved in the host (please provide yourself a means for backing up your files saved in the host) .
...
`docker build --tag=orboan/dcsss-httpd-wpdev .`
